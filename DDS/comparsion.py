...
def proposed_method_with_ppo(new_data, i, look_back, data_partition, cap):
    x = i
    data1 = new_data.loc[new_data['Month'].isin(x)]
    data1 = data1.reset_index(drop=True)
    data1 = data1.dropna()
    ...
    # 2. 引入 PPO 进行选择
    selected_IMFs = []
    for index, imf in ceemdan1.iteritems():
        reward = evaluate_imf(imf)  # 自定义评价函数，输出分数
        if reward > 0.5:  # PPO 选择策略：设定阈值
            selected_IMFs.append(imf)
        else:
            # 将直接用于最终预测的部分保存
            direct_predict = numpy.array(imf).reshape(-1, 1)

    # 3. 对选择的 IMF 进行二次分解（EWT）
    for imf in selected_IMFs:
        ewt, _, _ = ewtpy.EWT1D(imf, N=3)
        df_ewt = pd.DataFrame(ewt)
        df_ewt.drop(df_ewt.columns[2], axis=1, inplace=True)
        denoised = df_ewt.sum(axis=1, skipna=True)
        ceemdan1 = ceemdan1.drop(imf.name, axis=1)
        ceemdan1 = pd.concat([ceemdan1, denoised], axis=1)
    ...
    for col in ceemdan1:
        ...
        trainX = numpy.reshape(X, (X.shape[0], X.shape[1], 1))  # Adjusted for BiTCN
        testX = numpy.reshape(X1, (X1.shape[0], X1.shape[1], 1))  # Adjusted for BiTCN

        # 构建 BiTCN 模型
        model = Sequential()
        model.add(TCN(nb_filters=64, kernel_size=2, nb_stacks=1, dilations=[1, 2, 4, 8],
        ...
        optimizer = Adam(learning_rate=0.001)
        model.compile(loss='mse', optimizer=optimizer)
        model.fit(trainX, y, epochs=100, batch_size=64, verbose=0)

        # 预测结果
        y_pred_train = model.predict(trainX)
        y_pred_test = model.predict(testX)

        y_pred_test = sc_y.inverse_transform(y_pred_test.ravel().reshape(-1, 1))
        y_pred_train = sc_y.inverse_transform(y_pred_train.ravel().reshape(-1, 1))

        pred_test.append(y_pred_test)
        test_ori.append(sc_y.inverse_transform(y1.reshape(-1, 1)))
        pred_train.append(y_pred_train)
        train_ori.append(sc_y.inverse_transform(y.reshape(-1, 1)))

    # 汇总预测结果
    result_pred_test = pd.DataFrame(pred_test).sum(axis=0)
    result_pred_train = pd.DataFrame(pred_train).sum(axis=0)

    # 计算误差
    mape = numpy.mean((numpy.abs(test_ori - result_pred_test)) / test_ori) * 100
    rmse = sqrt(mean_squared_error(test_ori, result_pred_test))
    mae = numpy.mean(numpy.abs(test_ori - result_pred_test))
    r2 = r2_score(test_ori, result_pred_test)
...
# 自定义评价 IMF 的函数，用于 PPO 策略
def evaluate_imf(imf):
    # 根据 IMF 的能量、频率特征等进行评价
    energy = numpy.sum(imf ** 2)
    if energy > 0.1:  # 示例阈值
        return 0.7  # 示例分数
    else:
        return 0.3
